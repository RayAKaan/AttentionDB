...
//! quorum commitment, and physical engine command dispatching via TCP.
...
pub struct RaftNode {
    ...
    #[allow(dead_code)]
    mailbox_tx: Option<mpsc::UnboundedSender<RaftMessage>>,
    mailbox_rx: Option<mpsc::UnboundedReceiver<RaftMessage>>,
    /// Election timeout in milliseconds.
    pub election_timeout_ms: u64,
    /// Heartbeat interval in milliseconds (leaders only).
...
impl RaftNode {
    ...
    pub fn with_transport(self) -> Self {
        self
    }
    ...
    pub fn start_election(&mut self) -> Vec<RaftMessage> {
        ...
        for &peer in &self.peers {
            outgoing.push(RaftMessage {
                ...
                to: peer,
                term: self.current_term,
                payload: RaftPayload::RequestVote { last_log_index, last_log_term },
            });
        }
    ...
    /// Start the Raft node's async event loop.
    pub async fn start(&mut self) {
        let _rx = self.mailbox_rx.take()
            .expect("RaftNode already started");
        info!("[RaftNode {}] Starting async event loop (role={:?})", self.id, self.role);
        loop {
            tokio::select! {
                Some(msg) = recv_message(&self.peer_addresses, &self.id) => {
                    let responses = self.step(msg);
                    for resp in responses {
                        let _ = send_message(&self.peer_addresses, resp).await;
                    }
                }
                _ = tokio::time::sleep(tokio::time::Duration::from_millis(self.heartbeat_interval_ms)) => {
                    if self.role == RaftRole::Leader {
                        let heartbeats = self.broadcast_append_entries();
                        for hb in heartbeats {
                            let _ = send_message(&self.peer_addresses, hb).await;
                        }
                    }
                }
            }
...
/// Send a RaftMessage to the appropriate peer via TCP.
async fn send_message(
    addresses: &HashMap<u32, String>,
    msg: RaftMessage,
) -> Result<(), DistributedError> {
    let addr = addresses.get(&msg.to).ok_or_else(|| {
        DistributedError::Network(format!("Unknown peer {}", msg.to))
    })?;
    let mut stream = TcpStream::connect(addr).await
        .map_err(|e| DistributedError::Network(format!("TCP connect: {}", e)))?;
    let data = serde_json::to_vec(&msg)
        .map_err(|e| DistributedError::Network(format!("serialize: {}", e)))?;
    stream.write_all(&data).await
        .map_err(|e| DistributedError::Network(format!("TCP write: {}", e)))?;
    Ok(())
}

/// Receive a RaftMessage from a peer by listening on a local TCP port.
async fn recv_message(
    addresses: &HashMap<u32, String>,
    my_id: &u32,
) -> Option<RaftMessage> {
    let addr = addresses.get(my_id)?;
    let listener = tokio::net::TcpListener::bind(addr).await.ok()?;
    let (mut stream, _) = listener.accept().await.ok()?;
    let mut buf = Vec::new();
    stream.read_to_end(&mut buf).await.ok()?;
    let msg: RaftMessage = serde_json::from_slice(&buf).ok()?;
    Some(msg)
}
