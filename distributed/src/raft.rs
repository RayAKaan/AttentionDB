...
impl RaftNode {
    ...
    pub fn append_entry(&mut self, command: &str, data: Vec<u8>) -> Result<u64, DistributedError> {
        ...
        if self.role == RaftRole::Leader {
            self.match_index.insert(self.id, idx + 1);
        }
        Ok(idx)
    }
    ...
    pub fn broadcast_append_entries(&self) -> Vec<RaftMessage> {
        ...
        self.peers
            ...
            .map(|&p| {
                ...
                let prev_term = if prev > 0 && prev <= self.log.len() as u64 {
                    self.log[(prev - 1) as usize].term
                } else {
                    0
                };
                let entries = if next <= self.log.len() as u64 {
        ...
        match msg.payload {
            RaftPayload::RequestVote {
                ...
                out.push(RaftMessage {
                    ...
                    to: msg.from,
                    term: self.current_term,
                    payload: RaftPayload::RequestVoteResponse {
                        vote_granted: grant,
                    },
                });
            }
            ...
            RaftPayload::AppendEntries {
                ...
                if msg.term < self.current_term {
                    out.push(RaftMessage {
                        ...
                        to: msg.from,
                        term: self.current_term,
                        payload: RaftPayload::AppendEntriesResponse {
                            success: false,
                            match_index: 0,
                        },
                    });
                ...
                for e in entries {
                    ci += 1;
                    if ci > self.log.len() as u64 {
                        self.log.push(e);
                    } else {
                        self.log[(ci - 1) as usize] = e;
                    }
                }
            ...
            RaftPayload::AppendEntriesResponse {
                success,
                match_index,
            } => {
                if self.role == RaftRole::Leader && msg.term == self.current_term {
                    if success {
                        ...
                        if cur > 1 {
                            self.next_index.insert(msg.from, cur - 1);
    ...
    pub async fn start(self: Arc<Self>, listen_addr: String) {
        ...
        loop {
            match listener.accept().await {
                Ok((mut stream, _)) => {
                    let mut node = self.as_ref().clone();
                    tokio::spawn(async move {
                        if let Err(e) =
                            RaftTransport::handle_incoming_message(&mut node, &mut stream).await
                        {
...
mod tests {
    ...
    fn test_leader_election_quorum() {
        ...
        let c = cmds.clone();
        n3 = n3.with_commit_callback(move |e| {
            c.lock().push(e.command.clone());
        });
