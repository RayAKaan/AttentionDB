...
fn main() {
    ...
    match parse_aql(create_stmt) {
        ...
        _ => { println!("   Failed to parse CREATE COLLECTION"); return; }
    }
    ...
    let base_config = HNSWConfig {
        max_nb_connection: 16, ef_construction: 400, ef_search: 128,
        store_vectors: true, max_elements: 10_000,
    };
    ...
    match parse_aql(query_aql) {
        Ok(AQLStatement::Query(q)) => {
            ...
            for (id, s) in &sem { *fused.entry(*id).or_insert(0.0) += s; }
            for (id, s) in &tmp { *fused.entry(*id).or_insert(0.0) += s * 0.7; }
            ...
            println!("   {:>4}  {:>6}  {:>8}  {:>8}  {:>8}", "Rank", "ID", "Fused", "Sem", "Tmp");
            println!("   {}  {}  {}  {}  {}", "----", "------", "--------", "--------", "--------");
