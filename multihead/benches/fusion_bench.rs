...
fn fusion_benchmark(c: &mut Criterion) {
    ...
    let head_results = vec![
        ("semantic".to_string(), vec![(1, 0.92), (2, 0.85), (3, 0.71)]),
        ("temporal".to_string(), vec![(2, 0.88), (4, 0.79), (5, 0.67)]),
        ("structural".to_string(), vec![(1, 0.81), (3, 0.77), (6, 0.63)]),
    ];

    c.bench_function("gating_forward", |b| {
        b.iter(|| {
            manager.gating.forward(black_box(&query_emb))
        });
    });
    ...
    c.bench_function("fuse_scores", |b| {
        ...
        b.iter(|| {
            attentiondb_multihead::fuse_scores(black_box(&head_results), black_box(&gates))
        });
    });
