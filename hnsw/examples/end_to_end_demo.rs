...
fn main() {
    ...
    println!("   Indexed {n_docs} documents across all heads in {:.2?}\n", start.elapsed());
    ...
    let struct_sim: HashMap<u64, f32> = struct_results.iter().map(|(id, d)| (*id, sim(*d))).collect();
    ...
    for (&id, &s) in &sem_sim { *fused.entry(id).or_insert(0.0) += s * 1.0; }
    for (&id, &s) in &temp_sim { *fused.entry(id).or_insert(0.0) += s * 0.7; }
    for (&id, &s) in &struct_sim { *fused.entry(id).or_insert(0.0) += s * 0.5; }
    ...
    // Display
    println!("┌──────┬────────┬──────────┬──────────┬──────────┬──────────┬──────────────────────┐");
    println!("│ Rank │    ID  │    Fused │ Semantic │ Temporal │ Struct.  │ Topic                │");
    println!("├──────┼────────┼──────────┼──────────┼──────────┼──────────┼──────────────────────┤");
    ...
    for (rank, (id, score)) in ranked.iter().enumerate() {
        ...
        println!("│ {:<4} │ {:>6} │ {:>8.4} │ {:>8.4} │ {:>8.4} │ {:>8.4} │ {:<20} │",
                 rank + 1, id, score, sem, tmp, stc, topic);
    }
    println!("└──────┴────────┴──────────┴──────────┴──────────┴──────────┴──────────────────────┘\n");

    println!("▶ Per-head statistics:");
    println!("   Semantic:   {} total, returned {} results",
             semantic.len(), sem_results.len());
    println!("   Temporal:   {} total, returned {} results",
             temporal.len(), temp_results.len());
    println!("   Structural: {} total, returned {} results",
             structural.len(), struct_results.len());
